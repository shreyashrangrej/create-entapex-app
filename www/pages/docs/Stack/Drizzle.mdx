...
1. Standard Drizzle ORM config files that comes along with the tempalte.
...
```
2. ORM Client is initialized in index.ts
...
```
3. Schema File is where all your database schemas goes.
...
4. migrate.ts is a stand alone file to migrate the SQL migrations to database
